...
yum --installroot=$ROOTFS --nogpgcheck -y groupinstall base core server-policy
...
#grub config
KERNELVER=$(rpm --root=${ROOTFS} -qa | grep '^kernel-2.6.32')
...
title CentOS
  root(hd0)
  kernel /boot/vmlinuz-${KERNELVER} ro root=LABEL=/ console=hvc0 rd_NO_MD rd_NO_LVM rd_NO_DM
...
# AMI additions
for file in \
  http://www.bashton.com/downloads/centos-ami/RPMS/noarch/ec2-utils-0.3-1.15.el6_bashton2.noarch.rpm \
  http://www.bashton.com/downloads/centos-ami/RPMS/noarch/ec2-net-utils-0.3-1.15.el6_bashton2.noarch.rpm \
  ...
  http://www.bashton.com/downloads/centos-ami/RPMS/noarch/cloud-init-0.5.15-68.el6_bashton1.noarch.rpm ; do
  wget $file
  yum --installroot=$ROOTFS --nogpgcheck -y install $(basename $file)
  rm $(basename $file)
done
